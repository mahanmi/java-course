public enum Command {
    ...
    CREATE_CORP(
            "^create corps (?<infantry>\\d+) (?<cavalry>\\d+) (?<artillery>\\d+) (?<rankedOfficer>corporal|sergent|lieutenant|capitan|colonel|general|marshal) for (?<country>\\S+) (?<number>I||II||III||VI)$"),
    ...
    ADD_CORP_TO_ARMY("^add corps (?<corpsNumber>I||II||III||IV) to army (?<armyNumber>\\S+) of (?<country>\\S+)$"),
